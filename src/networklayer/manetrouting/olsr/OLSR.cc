...
// Kunz: trigger debug output based on defined compile-time constant
// #define HNA_OUTPUT
...
void OLSR_HnaTimer::expire() {
    // Kunz: Dynamically populate the SET of ASSOCIATIONS

    if (agent_->par("dynamicHNA").boolValue()) {
#ifdef HNA_OUTPUT
        cout << CURRENT_TIME << ": Routing agent " << agent_->ra_addr() << " adds HNA entries\n";
#endif
        agent_->associations().clear();
        IPv4Route *entry;
        ...
        for (int i=inet_rt->getNumRoutes()-1; i>=0; i--)
        {
            ...
            {
#ifdef HNA_OUTPUT
                cout << "To add: HNA for host " << entry->getDestination().str() << " ";
                cout << "with netmask " << entry->getNetmask().str() << "\n";
#endif
                OLSR_association* tuple = new OLSR_association;
                tuple->setNet_addr(nsaddr_t(entry->getDestination()));
                tuple->setNetmask(nsaddr_t(entry->getNetmask()));
                agent_->associations().push_back(tuple);
            }
        }
        // Missing: advertise reachability to own local interface (but could we route to it too?)
    }
...
void OLSR::rtable_computation() {
    ...
    if (par("DelOnlyRtEntriesInrtable_").boolValue()) {
        ...
        if(par("isFlooding")){
            ...
            // Kunz: not sure why this is needed, probably a Silas thing. But cleaning the WLAN0 interface routing table
            // entries SHOULD DEFINATELY NOT get deleted when we use the dynamicHNA approach, otherwise routes from the
            // gateways to nodes in a cluster disappear. So I added that as a requirement.
            if(!associations().empty() && !par("dynamicHNA").boolValue() && (getInterfaceTable()->getNumInterfaces() > 2)){
    ...
    // Kunz: print information about processing association set
#ifdef HNA_OUTPUT
    cout << CURRENT_TIME << ": Routing agent " << ra_addr_ << ": process association set of size " << associationset().size() << "\n";
#endif
    for (associationset_t::iterator it = associationset().begin();
...
#ifdef HNA_OUTPUT
        // Kunz: print information about tuple we are currently working on
        cout << "Address " << tuple->net_addr().str() << " ";
        cout << "Gateway " << tuple->gateway().str() << " " ;
        cout << "Netmask " << tuple->netmask().str() << " ";
        cout << "Validity Time " << tuple->time() << "\n";
#endif
        ...
        // Kunz: not sure why we should BREAK OUT of the loop when we processed the first ASSOCIATION, will uncomment it
        // probably a legacy from the fact that the code allowed multiple identical ASSOCIATIONS with different VTIMES
        // break;
    }
...
bool OLSR::process_hna(OLSR_msg& msg, const nsaddr_t &sender_iface,
...
#ifdef HNA_OUTPUT
    // Kunz: print information about received HNA message and check their processing: they should all
    // be added to the set of ASSOCIATION_TUPLES and then later used in determining routing table entries
    cout << now << ": Routing agent " << ra_addr_ << ": ";
    cout << "Received " << hna.count << " HNA entries\n";
    for (int i = 0; i < hna.count; i++) {
        cout << "Entry " << i << " " << hna.net_addr(i) << " " << msg.orig_addr() << " " << hna.netmask(i) << " ";
        cout << "Validity time " << emf_to_seconds(msg.vtime()) << "\n";
    }
#endif
    ...
    for (int i = 0; i < hna.count; i++) {
        ...
        // Kunz: changed the whole logic of checking against the ASSOCIATION_SET
        // 1) create a tuple with the information before calling find_association_tuple()
        // 2) changed how find_association_tuple() looks for the tuple, see OLSR_state.cc
        // 3) if tuple is found, simply update validity time and reschedule associated timer
        //    - rescheduling the timer was missing in the code
        // 4) if it is a new tuple, do not simply create a timer, but save reference to it
        ...
        OLSR_association_tuple* found = state_.find_association_tuple(tuple);
        if (found != NULL) {
            found->time() = now + OLSR::emf_to_seconds(msg.vtime());
            // reschedule associated timer
            OLSR_AssociationTupleTimer* tuple_timer = (OLSR_AssociationTupleTimer*)found->asocTimer;
            tuple_timer->resched(DELAY(found->time()));
        } else {
            ...
            tuple->asocTimer = tuple_timer;
        }

    }
#ifdef HNA_OUTPUT
    // Kunz: print out set of ASSOCIATION TUPLES
    cout << now << ": Routing agent " << ra_addr_ << " association set (" << associationset().size() << "):\n";
    for (int i = 0; i < associationset().size(); i++) {
        cout << associationset()[i]->net_addr().str() << " ";
        cout << associationset()[i]->gateway().str() << " " ;
        cout << associationset()[i]->netmask().str() << " ";
        cout << associationset()[i]->time() << "\n";
    }
#endif
    return false;
