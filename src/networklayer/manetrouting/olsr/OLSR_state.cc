...
OLSR_state::find_association_tuple (const nsaddr_t &gate_addr, const nsaddr_t &net_addr, const nsaddr_t &netmask)

{
  for (associationset_t::iterator it = associationset_.begin ();
       it != associationset_.end (); it++)
    {
      ...
      if (tuple->gate_addr() == gate_addr && tuple->net_addr() == net_addr && tuple->netmask() == netmask)
